...
- **控件名优先**:用控件中文名或英文名命中。例:`底部页签`、`弹窗`、`按钮`、`选择器`。
- **同义词已内置扩展(本库核心优势)**:弹窗↔弹出框↔对话框↔dialog、开关↔toggle、列表↔list、菜单↔menu、搜索↔search、导航↔navigation 等自动 OR 扩展,用任一写法都能命中(如查"弹窗"能命中标题为"弹出框"的文档)。
- **视觉规范词**:色彩、字体、排版、圆角、间距、动效、转场、暗色模式/深色模式。
- **多设备适配词**:折叠屏、平板、穿戴、2in1、响应式布局。
- **查不到时**:先 `list_design_guides_by_topic` 看大类(尤其"控件"类),再下钻到子类(如 `控件 / 导航类`)。

## 大类清单(先定位,再下钻)

166 篇设计指南,9 个顶级类:

通用设计基础、控件(49)、针对多设备设计(26)、系统特性&能力(22)、应用设计最佳实践(14)、元服务设计(12)、人机交互(11)、应用 UX 体验标准(8)、变更说明(1)。

用 `list_design_guides_by_topic({topic:"<大类>"})` 下钻,支持多级路径前缀(如 `控件 / 导航类`)。
